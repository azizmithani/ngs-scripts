...
int nNoHit = 0;
int nSingleHit = 0;
int nMultipleHitSameFrame = 0;
int nMultipleHitDiffFrame = 0;
...
void WriteHSP(ofstream& ofs, string Id, HIT& Hit, HSP& Hsp) {
	ofs << Id << "\t" << Hit.HitId << "\t" << Hit.HitDef << "\t" << Hsp.HspFrom << "\t" << Hsp.HspTo << "\t" << Hsp.HspFrame << endl;
}
...
//			ofs << Id << "\t-\t-\t" << qStart << "\t" << qEnd << "\t" << frame << endl;
			...
			if (Hit->Hsps.size() == 1) { // only one hsp found for this hit (majority scenario)
				...
				WriteHSP(ofs, Id, *Hit, *Hit->Hsps.begin());
				
				// increment the count
				nSingleHit++;
			} else {
				// check if all hsps are in the same frame.
				if ( CheckHSPFrames(Hit->Hsps) ) {
					// same frame .. merge the Hsps
					int frame = Hit->Hsps.begin()->HspFrame;
					HSP newHsp;
					MergeHsps(Hit->Hsps, newHsp, frame);
					CheckHSP(dna, newHsp);
					WriteHSP(ofs, Id, *Hit, newHsp);
					// put the new Hsp in the list of Hsps
					Hit->Hsps.push_back(newHsp);
					
					// increment the count
					nMultipleHitSameFrame++;
				} else {
					// multiple frames .. merge the Hsps belonging to the same frame
					vector<HSP> newHsps;
					for (int frame = 1; frame <= 3; frame++) {
						HSP newHsp;
						if (!MergeHsps(Hit->Hsps, newHsp, frame)){
							continue;
						}
...
int main (int argc, char** argv) {
	...
	cout << "Multiple Hits - Same Frame: " << nMultipleHitSameFrame << endl;
	cout << "Multiple Hits - Different Frames: " << nMultipleHitDiffFrame << endl;
	// close the stream
	ofs.close();
