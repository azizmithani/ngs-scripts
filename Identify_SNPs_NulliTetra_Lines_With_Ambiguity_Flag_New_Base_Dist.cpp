...
void ReadBaseDistributionFile(string Base_Dist_File, BASE_DIST_LIST& BaseDistList) {
	...
	while (ifs_base_dist.good()) {
		...
		if (str.empty()) { // ignore emtry lines
			...
			RefBaseDistList = new int* [length];
			for(int i = 0; i < length; i++) {
				*(RefBaseDistList+i) = new int[TOTAL_BASES];		
				for (int j = 0; j < TOTAL_BASES; j++) {
					(*(RefBaseDistList+i))[j] = 0;
				}
			}
			
			BaseDistList [reference] = RefBaseDistList;
		} else {
			vector<string> entries;
			strsplit(str, entries, "\t");
			...
			if (current_reference.compare(reference) != 0) {
				RefBaseDistList = BaseDistList[reference];
				current_reference = reference;
			}

//			if (position == 10249221) {
//				cout << position << "\t" << str << endl;
//			}
			
			int* BaseFreq = *(RefBaseDistList + position - 1);
			for ( int i = 2; i < entries.size(); i++ ) {
				BaseFreq[i-2] = atoi(entries[i].c_str());
			}
//			if (position == 10249221) {
//				cout << position << "\t" << BaseFreq[0] << "\t" << BaseFreq[1] << "\t" << BaseFreq[2]  << "\t" << BaseFreq[3]  << "\t" << BaseFreq[4]  << "\t" << BaseFreq[5] << endl;
//			}
		}
	}
	ifs_base_dist.close();
...
void CheckBaseDistribution(map <string, string>& BaseDistributionsFiles, map <string, string>& NT_SNP_Files, SNP_TABLE& NT_SNPs){
	...
	for (; itNT_SNP_Files != NT_SNP_Files.end(); itNT_SNP_Files++) {
		...
		for (; itNT_SNPs != NT_SNPs.end(); itNT_SNPs++) {
			...
			reference = SNPRow[0];
			int position = atoi(SNPRow[1].c_str());
			...
			int* BaseDist = RefBaseDistList[position - 1];
			// calculate coverage for this position
			double Coverage = (double)CalculateCoverage(BaseDist);

//			if (SNPRow[1].compare("10249221") == 0) {
//				cout << position << "\t" << Base << "\t" << Coverage << endl;
//			}
			...
			for (int b = 0; b < TOTAL_BASES; b++) {
				if (BaseDist[b] >= MIN_BASE_COVERAGE && (double)BaseDist[b]/Coverage >= MIN_BASE_PROPORTION) {
					...
					string theBase (1, GetIndexBase(b));
					NewBases.insert(theBase);
				}
			}
//			if (SNPRow[1].compare("10249221") == 0) {
//				cout << position << "\t" << SNPRow[2] << "\t" << SetToString(NewBases) << endl;
//			}
...
//			if (SNPRow[1].compare("10249221") == 0) {
//				cout << idxNT << "\t" << SNPRow[2] << "\t" << SNPRow[idxNT + OFFSET] << "\t" <<  Base << endl;
//			}			
