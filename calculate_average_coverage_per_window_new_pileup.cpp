...
void CalculateCoverage(string Header_File, string Pileup_File, string OUT_File) {
	...
	while (ifs_pileup.good()) {
		...
		if (strPileup.empty()) { // ignore emtry lines
			continue;
		} else if (strPileup[0] == '#') { // ignore comments
			continue;
		} else {
			vector<string> entries;
			...
			string info = entries[7];
			int pos1 = info.find("DP=");
			int pos2 = info.find(";",pos1);
			
			int coverage = atoi(info.substr(pos1+3,pos2).c_str());
			
			int window = pos / WINDOW_SIZE;
			*(RefCoverage + window) = *(RefCoverage + window) + coverage;
